...
func NewClient(clientId int, nodeIds []int, recvChan <-chan Message, sendChan chan<- Message, sendIntvMS int) Client {
	sendIntv := time.Millisecond * time.Duration(sendIntvMS)
	log.Printf("C%d, Send Interval: %d milliseconds", clientId, sendIntvMS)
	return Client{clientId, NewClockVal(nodeIds), recvChan, sendChan, sendIntv, 0, make([]Message, 0)}
}
